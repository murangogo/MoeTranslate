...
**本地OCR识别+文本翻译：** 支持的API有（共11个）：
- 两种本地AI翻译：ML Kit翻译、NLLB翻译；
- 六种预置云端API：必应翻译、小牛翻译、火山引擎、Azure翻译、百度翻译、腾讯云；
- 三个自定义云端文本翻译API预留位。
...
预置云端API有六种，分别是必应翻译、小牛翻译、火山引擎、Azure翻译、百度翻译、腾讯云。其中只有必应翻译不需要配置API Key，其他的云端API均需配置API Key。
...
除了上面介绍的必应翻译，其他的API均需配置API Key，其配置过程大同小异，下面以翻译效果最好的小牛翻译为例，解释一下流程。
...
预置云端API有两种，分别是百度翻译、腾讯云，均需配置API Key。
