...
必应翻译不需要配置API Key，其实现原理是通过模拟浏览器访问网页的行为来实现翻译，依赖于网页结构和token提取，这是非常不规范的行为，稳定性较差。也正因为如此，萌译无法保证它随时可用，还是**推荐您使用其他翻译API，如[聚合AI翻译](https://www.moetranslate.top/docs/translationapi/uniaitrans/)或[小牛翻译](https://www.moetranslate.top/docs/translationapi/niutrans/)**。
