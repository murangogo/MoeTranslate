 ...
 * タッチマネージャー
 */
public class TouchManager {     //管理触摸的类
    /**
     * タッチ開始時のxの値
     */
     ...
     */
    public void touchesBegan(float deviceX, float deviceY) {    //获得触摸坐标
        lastX = deviceX;        //记录最新的坐标
        lastY = deviceY;        //记录最新的坐标

        startX = deviceX;   //记录开始坐标
        startY = deviceY;   //记录开始坐标，实际上意思是点击

        lastTouchDistance = -1.0f;

        isFlipAvailable = true;
        isTouchSingle = true;   //点击一次
    }
     ...
     * @param deviceY タッチした画面のyの値
     */
    public void touchesMoved(float deviceX, float deviceY) {        //更新坐标
        lastX = deviceX;
        lastY = deviceY;
        lastTouchDistance = -1.0f;
        isTouchSingle = true;
